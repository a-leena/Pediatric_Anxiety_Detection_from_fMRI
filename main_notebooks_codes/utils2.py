...
def get_maps_tables(sub_info, get_task=True, get_task_mean=True, get_rest=True, get_all_confounds_tables=True, get_probmaps=False, get_anat=False):
    # Paths
    ...
    if get_task:
        ...
        if get_all_confounds_tables:
            run1_confounds_path = os.path.join(func_path, f'all-confounds_{sub_info['subject_id']}_ses-1_task-TAU{sub_info['cohort']}_run-1.csv')
            run2_confounds_path = os.path.join(func_path, f'all-confounds_{sub_info['subject_id']}_ses-1_task-TAU{sub_info['cohort']}_run-2.csv')
        else:
    ...
    if get_rest:
        if sub_info['cohort']==1:
            ...
            if get_all_confounds_tables:
                rest_confounds_path = os.path.join(func_path, f'all-confounds_{sub_info['subject_id']}_ses-1_task-rest.csv')
            else:
    ...
    if get_task:
        ...
        if get_all_confounds_tables:
            run1_confounds_table = pd.read_csv(run1_confounds_path)
            run2_confounds_table = pd.read_csv(run2_confounds_path)
        else:
    ...
    if get_rest:
        if get_all_confounds_tables:
            rest_confounds_table = pd.read_csv(rest_confounds_path)
        else:
            if sub_info['cohort']==1:
    ...
    if get_task and get_task_mean and not get_rest and get_probmaps and not get_anat:
        return [run1, run2], mean_fmri, [run1_events_table, run2_events_table], [run1_confounds_table, run2_confounds_table], wm_probmap, csf_probmap
    ...
    if get_task and get_task_mean and get_rest and get_probmaps and not get_anat:
        return [run1, run2], mean_fmri, rest, [run1_events_table, run2_events_table], [run1_confounds_table, run2_confounds_table], rest_confounds_table, wm_probmap, csf_probmap
    ...
    if get_task and get_task_mean and get_rest and get_probmaps and get_anat:
        return [run1, run2], mean_fmri, rest, anat, [run1_events_table, run2_events_table], [run1_confounds_table, run2_confounds_table], rest_confounds_table, wm_probmap, csf_probmap
    ...
    if get_task_mean and get_rest and get_probmaps and not get_anat:
        return mean_fmri, rest, rest_confounds_table, wm_probmap, csf_probmap
    ...
    if get_task_mean and get_rest and get_probmaps and get_anat:
        return mean_fmri, rest, anat, rest_confounds_table, wm_probmap, csf_probmap
...
def get_confounds(wm_probmap, csf_probmap, mean_fmri, for_task=True, for_rest=True, other_task_confounds_dfs=[], other_rest_confounds_df=None, task_fmri_runs=[], rest_fmri=None, threshold=0.9):
    ...
    if for_task and for_rest:
        return task_confounds_dfs, rest_confounds_df
...
def get_fl_contrast_map(sub_info, contrast_name, has_all_confounds, verbose=0):
    # get subject task scans & tables
    if has_all_confounds:
        runs, mean_fmri, runs_events_tables, runs_confounds_tables = get_maps_tables(
            sub_info, get_task=True, get_task_mean=True, get_rest=False,
            get_all_confounds_tables=True, get_probmaps=False, get_anat=False
        )
        task_confounds_tables = runs_confounds_tables
    else:
        runs, mean_fmri, runs_events_tables, runs_confounds_tables, wm_probmap, csf_probmap = get_maps_tables(sub_info, get_task=True, get_task_mean=True, get_rest=False, get_all_confounds_tables=False, get_probmaps=True, get_anat=False)
...
def get_group_mean_maps(sub_list, phenotype_table, has_all_confounds, fl_contrast_name='incongruent_vs_congruent', fl_contrast_map_verbose=0, sl_verbose=0, show_sl_design_matrix=False, save_group_mean_tfmri=False, save_group_mean_tfmri_path=HOLDOUTS_PATH, save_fl_contrasts=False, save_fl_contrasts_path=HOLDOUTS_PATH, save_sl_design_matrix=False, save_sl_design_matrix_path=HOLDOUTS_PATH, save_mean_task_zmap=False, save_mean_task_zmap_path=HOLDOUTS_PATH):
    ...
    try:
        ...
        for sub_info in subs_info:
            ...
            sub_contrast_map = get_fl_contrast_map(
                sub_info=sub_info, 
                contrast_name=fl_contrast_name, 
                has_all_confounds=has_all_confounds, 
                verbose=fl_contrast_map_verbose
            )
...
def get_task_rest_features(sub_info, has_all_confounds, group_ROIs_mask, group_ROIs_coords, fl_contrast_name, fl_alt_test=None, task_zmap=None, ROIs_task_zmap=None, ROIs_rest_FC_matrix=None, show_zmap=False, show_ROIs_zmap=False, show_ROIs_connectome=False, coords=[0,0,0], figsize=(8,4)):
    # get subject scans
    if has_all_confounds:
        runs, mean_fmri, rest, runs_events_tables, runs_confounds_tables, rest_confounds_table = get_maps_tables(sub_info, get_task=True, get_task_mean=True, get_rest=True, get_all_confounds_tables=True, get_probmaps=False, get_anat=False)
        task_confounds_tables = runs_confounds_tables
        rest_confounds_table_final = rest_confounds_table
    else:
        runs, mean_fmri, rest, runs_events_tables, runs_confounds_tables, rest_confounds_table, wm_probmap, csf_probmap = get_maps_tables(sub_info, get_task=True, get_task_mean=True, get_rest=True, get_all_confounds_tables=False, get_probmaps=True, get_anat=False)
        # get complete confounds tables
        task_confounds_tables, rest_confounds_table_final = get_confounds(
            ...
            task_fmri_runs=runs, rest_fmri=rest, 
            threshold=WM_CSF_PROB_THRESH
        )
