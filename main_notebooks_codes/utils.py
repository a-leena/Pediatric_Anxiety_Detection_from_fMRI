...
def get_maps_tables(sub_info, get_task=True, get_task_mean=True, get_rest=True, get_anat=False):
    # Paths
    ...
    if get_task and get_task_mean and not get_rest and not get_anat:
        return run1, run2, mean_fmri, run1_events_table, run2_events_table, run1_confounds_table, run2_confounds_table
    if get_task and get_task_mean and get_rest and not get_anat:
    ...
    if get_task and get_task_mean and get_rest and get_anat:
    ...
    if get_task_mean and not get_rest and not get_anat:
        return mean_fmri
    if get_task_mean and get_rest and not get_anat:
        return mean_fmri, rest, rest_confounds_table
    if get_task_mean and get_rest and get_anat:
...
def get_fl_contrast_map(sub_info, contrast_name, verbose=0):
    ...
    run1, run2, mean_fmri, run1_events_table, run2_events_table, run1_confounds_table, run2_confounds_table = get_maps_tables(sub_info, get_task=True, get_rest=False)
    if verbose==1:
        print("Fetched task-fMRI scans and events & confounds tables")

    # perform first-level GLM fitting
    fl_glm = fit_fl_glm([run1, run2], [run1_events_table, run2_events_table], [run1_confounds_table, run2_confounds_table])
    if verbose==1:
        print("Fitted First-level GLM")

    # perform contrast testing
...
def get_group_mean_task_fmri(subs_info, save_img=False, save_path='..'):
    ...
    for sub_info in subs_info:
        mean_fmri = get_maps_tables(sub_info, get_task=False, get_task_mean=True, get_rest=False, get_anat=False)
...
def get_task_rest_features(sub_info, group_ROIs_mask, group_ROIs_coords, fl_contrast_name, fl_alt_test=None, task_zmap=None, ROIs_task_zmap=None, ROIs_rest_FC_matrix=None, show_zmap=False, show_ROIs_zmap=False, show_ROIs_connectome=False, coords=[0,0,0], figsize=(8,4)):
    # get subject scans
    run1, run2, mean_fmri, rest, run1_events_table, run2_events_table, run1_confounds_table, run2_confounds_table, rest_confounds_table = get_maps_tables(sub_info)
    ...
    if ROIs_rest_FC_matrix is None:
        ...
        ROIs_rest_FC_matrix = get_masked_rs_fc(rest, rest_confounds_table, group_ROIs_mask)
